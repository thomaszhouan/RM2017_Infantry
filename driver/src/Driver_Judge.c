...
void JUDGE_Decode(uint32_t length) {
    ...
    if (frameByteCount == JUDGE_INFO_FRAME_LENGTH) {
        ...
        FT.U[0] = JUDGE_DataBuffer[13];
        FT.U[1] = JUDGE_DataBuffer[14];
        FT.U[2] = JUDGE_DataBuffer[15];
        FT.U[3] = JUDGE_DataBuffer[16];
        JUDGE_Data.voltage = FT.F;

        FT.U[0] = JUDGE_DataBuffer[17];
        FT.U[1] = JUDGE_DataBuffer[18];
        FT.U[2] = JUDGE_DataBuffer[19];
        FT.U[3] = JUDGE_DataBuffer[20];
        JUDGE_Data.current = FT.F;

        FT.U[0] = JUDGE_DataBuffer[34];
        FT.U[1] = JUDGE_DataBuffer[35];
        FT.U[2] = JUDGE_DataBuffer[36];
        FT.U[3] = JUDGE_DataBuffer[37];
        JUDGE_Data.remainEnergy = FT.F;

        JUDGE_Data.remainLife = ((uint16_t)JUDGE_DataBuffer[12]<<8) | JUDGE_DataBuffer[11];
...
#else // use simulated judge
    if (frameByteCount == 10) {
        ++JUDGE_FrameCounter;

        FT.U[0] = JUDGE_DataBuffer[2];
        FT.U[1] = JUDGE_DataBuffer[3];
        FT.U[2] = JUDGE_DataBuffer[4];
        FT.U[3] = JUDGE_DataBuffer[5];
        JUDGE_Data.voltage = FT.F;
        ...
        JUDGE_Data.remainLife = ((uint16_t)JUDGE_DataBuffer[1]<<8) | JUDGE_DataBuffer[0];
...
uint16_t Get_CRC16_Check_Sum(uint8_t *pchMessage,
    uint32_t dwLength,uint16_t wCRC) {
    uint8_t chData;
    if (pchMessage == 0)
        return 0xFFFF;
    while(dwLength--) {
